...
TransverseMercator::TransverseMercator(real a, real f, real k0, bool exact, bool extendp)
    : _a(a),
      _f(f),
      _k0(k0),
      _exact(exact),
      ...
      _n(_f / (2 - _f)),
      _tmexact(_exact ? TransverseMercatorExact(a, f, k0, extendp) : TransverseMercatorExact()) {
    if (_exact)
        return;
    if (!(isfinite(_a) && _a > 0))
...
#error "Bad value for GEOGRAPHICLIB_TRANSVERSEMERCATOR_ORDER"
    ...
    static_assert(sizeof(alpcoeff) / sizeof(real) == (maxpow_ * (maxpow_ + 3)) / 2,
                  "Coefficient array size mismatch for alp");
    static_assert(sizeof(betcoeff) / sizeof(real) == (maxpow_ * (maxpow_ + 3)) / 2,
...
const TransverseMercator& TransverseMercator::UTM() {
    static const TransverseMercator utm(Constants::WGS84_a(), Constants::WGS84_f(),
                                        Constants::UTM_k0());
    return utm;
}
...
void TransverseMercator::Forward(real lon0, real lat, real lon, real& x, real& y, real& gamma,
                                 real& k) const {
    if (_exact)
    ...
    int latsign = signbit(lat) ? -1 : 1, lonsign = signbit(lon) ? -1 : 1;
    lon *= lonsign;
    lat *= latsign;
    ...
    if (lat != Math::qd) {
        ...
        // instead of psip).
        k = sqrt(_e2m + _e2 * Math::sq(cphi)) * hypot(real(1), tau) / hypot(taup, clam);
    } else {
        xip = Math::pi() / 2;
        etap = 0;
        gamma = lon;
...
void TransverseMercator::Reverse(real lon0, real x, real y, real& lat, real& lon, real& gamma,
                                 real& k) const {
    if (_exact)
    ...
    // Explicitly enforce the parity
    int xisign = signbit(xi) ? -1 : 1, etasign = signbit(eta) ? -1 : 1;
    xi *= xisign;
    eta *= etasign;
    ...
    //   psi = asinh(tan(phi'))
    real xip = y1.real(), etap = y1.imag(), s = sinh(etap),
    ...
    if (r != 0) {
        ...
        k *= sqrt(_e2m + _e2 / (1 + Math::sq(tau))) * hypot(real(1), tau) * r;
    } else {
        lat = Math::qd;
        lon = 0;
        k *= _c;
