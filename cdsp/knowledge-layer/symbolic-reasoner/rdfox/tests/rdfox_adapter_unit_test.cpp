...
TEST(RDFoxAdapterTest, CheckNonexistentDatastore) {
    ...
    EXPECT_CALL(mock_adapter_, sendRequest(http::verb::get, "/datastores", "", "",
                                           "text/csv; charset=UTF-8", testing::_))
        .WillOnce(testing::DoAll(testing::SetArgReferee<5>(""), testing::Return(false)));

    EXPECT_FALSE(mock_adapter_.checkDataStore());
}
...
TEST(RDFoxAdapterTest, CheckExistentDatastore) {
    ...
    EXPECT_TRUE(mock_adapter_.checkDataStore());
}
...
TEST(RDFoxAdapterTest, InitializationCreatesDatastore) {
    ...
    EXPECT_CALL(mock_adapter_, sendRequest(http::verb::post, "/datastores/test_ds", "",
                                           "application/json", "", testing::_))
        .WillOnce(testing::Return(true));

    EXPECT_NO_THROW(mock_adapter_.initialize());
}
/**
...
TEST(RDFoxAdapterTest, LoadDataSuccess) {
    ...
    EXPECT_CALL(mock_adapter_, sendRequest(http::verb::post, "/datastores/test_ds/content",
                                           ttl_data, "text/turtle", "", testing::_))
        .WillOnce(testing::Return(true));

    EXPECT_TRUE(mock_adapter_.loadData(ttl_data));
}
...
TEST(RDFoxAdapterTest, QueryDataSuccess) {
    ...
    EXPECT_CALL(mock_adapter_,
                sendRequest(http::verb::post, "/datastores/test_ds/sparql", sparql_query,
                            "application/sparql-query", "", testing::_))
        .WillOnce(testing::DoAll(testing::SetArgReferee<5>(mock_response), testing::Return(true)));

    EXPECT_EQ(mock_adapter_.queryData(sparql_query), mock_response);
}
...
TEST(RDFoxAdapterTest, DeleteDataStoreSuccess) {
    ...
    EXPECT_TRUE(mock_adapter_.deleteDataStore());
}
...
TEST(RDFoxAdapterTest, FailedToCreateDataStore) {
    ...
    EXPECT_THROW(mock_adapter_.initialize(), std::runtime_error);
}
...
TEST(RDFoxAdapterTest, LoadDataFailure) {
    ...
    EXPECT_FALSE(mock_adapter_.loadData(ttl_data));
}
...
TEST(RDFoxAdapterTest, QueryDataFailure) {
    ...
    EXPECT_EQ(mock_adapter_.queryData(sparql_query), "");
}
...
TEST(RDFoxAdapterTest, DeleteDataStoreFailure) {
    ...
    EXPECT_FALSE(mock_adapter_.deleteDataStore());
}