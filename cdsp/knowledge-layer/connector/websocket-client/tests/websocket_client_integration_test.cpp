...
TEST_F(WebSocketClientIntegrationTest, FullFlowTest) {
    ...
    mock_connection_->addReceivedMessage(
        createDataJsonMessage(init_config_.oid[SchemaType::VEHICLE], nodes));
    ...
    // Verify RDFox Data
    std::string sparql_query(createObservationQuery(init_config_.oid[SchemaType::VEHICLE],
                                                    "Vehicle", "speed", nodes.at(0).value, "float",
                                                    start_time, end_time));

    // Assert data exists in RDFox
    std::string result = rdfox_adapter_->queryData(sparql_query, DataQueryAcceptType::TEXT_CSV);
...
TEST_F(WebSocketClientIntegrationTest, FullFlowTestMultiNodes) {
    ...
    mock_connection_->addReceivedMessage(
        createDataJsonMessage(init_config_.oid[SchemaType::VEHICLE], nodes_msg_1));
    ...
    mock_connection_->addReceivedMessage(
        createDataJsonMessage(init_config_.oid[SchemaType::VEHICLE], nodes_msg_2));
    ...
    std::string sparql_query_msg_1_qry_1(createObservationQuery(
        init_config_.oid[SchemaType::VEHICLE], "StateOfCharge", "currentEnergy",
        nodes_msg_1.at(1).value, "float", start_time, end_time));

    std::string sparql_query_msg_1_qry_2(
        createObservationQuery(init_config_.oid[SchemaType::VEHICLE], "Vehicle", "speed",
                               nodes_msg_1.at(0).value, "float", start_time, end_time));

    // Verify RDFox Data Nodes for second message
    std::string sparql_query_msg_2_qry_1(
        createObservationQuery(init_config_.oid[SchemaType::VEHICLE], "SteeringWheel", "angle",
                               nodes_msg_2.at(0).value, "int", start_time, end_time));
